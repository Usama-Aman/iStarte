...
import com.minbio.erp.financial_management.accounting.fragments.reporting.turn_over_collected.models.TOCollectedModel
import com.minbio.erp.financial_management.accounting.fragments.reporting.turn_over_invoiced.TurnOverInvoicedAdapter
...
class TurnOverCollectedFragment : Fragment(), ResponseCallBack {

    private lateinit var v: View
    private lateinit var fatocRecyclerView: RecyclerView
    private lateinit var turnOverCollectedAdapter: TurnOverCollectedAdapter
    ...
    private var toCollectedData: MutableList<TOCollectedData> = ArrayList()
    ...
    override fun onCreateView(
        ...
        v = inflater.inflate(
            R.layout.fragment_fa_turnover_collected,
            container,
            false
        )
    ...
    private fun initViews() {
        fatocRecyclerView = v.findViewById(R.id.fatocRecyclerView)
    ...
    private fun setUpAdapter() {
        turnOverCollectedAdapter = TurnOverCollectedAdapter(toCollectedData)
        fatocRecyclerView.layoutManager =
            LinearLayoutManager(context, LinearLayoutManager.HORIZONTAL, false)
        fatocRecyclerView.adapter = turnOverCollectedAdapter
    }

    private fun getCollectedTurnOver() {
        AppUtils.showDialog(context!!)
        val call = RetrofitClient.getClient(context!!).create(Api::class.java)
            .getTurnOverCollected(fromDateToSearch, toDateToSearch)
        RetrofitClient.apiCall(call, this, "TurnOverCollected")
    }

    override fun onSuccess(jsonObject: JSONObject, tag: String) {
        AppUtils.dismissDialog()
        val model = Gson().fromJson(jsonObject.toString(), TOCollectedModel::class.java)
        toCollectedData.clear()
        toCollectedData.addAll(model.data!!)

        if (toCollectedData.isNullOrEmpty()) {
            fatocRecyclerView.visibility = View.GONE
            tvNoData.visibility = View.VISIBLE
        } else {
            fatocRecyclerView.visibility = View.VISIBLE
            tvNoData.visibility = View.GONE
        }

        turnOverCollectedAdapter.notifyDataSetChanged()
