...
import com.minbio.erp.customer_management.adapter.CustomerCreditNoteAdapter
...
class CustomerCreditNote : Fragment(), ResponseCallBack {
    ...
    private lateinit var customerCreditNoteAdapter: CustomerCreditNoteAdapter
    private lateinit var cnRecyclerView: RecyclerView
    ...
    override fun onCreateView(
        ...
        v = inflater.inflate(R.layout.fragment_customer_credit_note, container, false)
    ...
    private fun initViews() {
        ...
        cnRecyclerView = v.findViewById(R.id.cn_recycler_view)
    ...
    private fun setAdapter() {
        customerCreditNoteAdapter = CustomerCreditNoteAdapter(cdBalanceList)
        cnRecyclerView.layoutManager = LinearLayoutManager(context)
        cnRecyclerView.adapter = customerCreditNoteAdapter

    }

    private fun initScrollListener() {
        cnRecyclerView.addOnScrollListener(object :
    ...
    private fun loadMore() {
        try {
            if (CURRENT_PAGE < LAST_PAGE) {
                ...
                customerCreditNoteAdapter.notifyItemInserted(cdBalanceList.size - 1)
    ...
    private fun handleOrderResponse(jsonObject: JSONObject) {
        ...
        if (cdBalanceList.size > 0) {
            ...
            customerCreditNoteAdapter.notifyItemRemoved(cdBalanceList.size)
        }
        ...
        if (cdBalanceList.isNullOrEmpty()) {
            ...
            cnRecyclerView.visibility = View.GONE
        } else {
            tvNoData.visibility = View.GONE
            cnRecyclerView.visibility = View.VISIBLE
        }

        customerCreditNoteAdapter.notifyDataSetChanged()
        isLoading = false
    }
