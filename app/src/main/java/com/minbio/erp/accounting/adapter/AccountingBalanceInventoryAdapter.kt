...
class AccountingBalanceInventoryAdapter(_inventoryList: MutableList<InventoryListData?>) :
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        ...
        return if (viewType == 0) Items(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_accounting_balance_inventory, parent, false)
        ) else
            Progress(
                LayoutInflater.from(parent.context)
    ...
    inner class Items(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        var buying_cost: TextView = itemView.findViewById(R.id.buying_cost)

        fun bind(position: Int) {
            txt_lot_no.text = inventoryList[position]?.lot_no
            ...
            buying_cost.setText(inventoryList[position]?.buying_cost.toString())
        }
    }
